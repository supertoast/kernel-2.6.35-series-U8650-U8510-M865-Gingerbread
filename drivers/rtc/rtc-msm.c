...
extern struct mutex alarm_elapsedrealtime_mutex;
/* apply qualcomm path about kernel time
 */
#endif
...
static void process_cb_request(void *buffer)
{
	...
	if (rtc_cb->event == EVENT_TOD_CHANGE) {
		...
		/* we use mutex to make sure the alarm_update_timedelta
		 * and the alarm_get_elapsed_realtime does not executes
		 * at the same time to fix a bug that cause the elapsedRealtime 
		 * goes backward
		 */
		#ifdef CONFIG_HUAWEI_KERNEL
/* apply qualcomm path about kernel time
 */
		mutex_lock(&alarm_elapsedrealtime_mutex);
		#endif
		getnstimeofday(&ts);
/* apply qualcomm path about kernel time
 */
		...
		#ifdef CONFIG_HUAWEI_KERNEL
		mutex_unlock(&alarm_elapsedrealtime_mutex);
		#endif
	} else
...
msmrtc_probe(struct platform_device *pdev)
{
	...
	//memcpy(&huawei_alarm_pdata,rtc_pdata,sizeof(struct msm_rtc));
