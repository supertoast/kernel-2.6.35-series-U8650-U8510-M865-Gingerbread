...
#ifdef CONFIG_HUAWEI_KERNEL
	ret = 0;
	if( (LCD_HX8368A_SEIKO_QVGA != lcd_panel_probe()) && 
        (LCD_R61529_TRULY_HVGA != lcd_panel_probe()) )
	{
...
#endif
/* we changed the order of the lcdc_off function 
 * to let the PLCK lasts at least 2 frames after the Sleep_In command
 * send to the LCD panel to ensure the LCD enter sleep mode
 */
#ifdef CONFIG_HUAWEI_KERNEL
