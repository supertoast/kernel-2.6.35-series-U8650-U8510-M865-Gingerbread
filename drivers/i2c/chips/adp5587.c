...
static void adp5587_work_fuc(struct work_struct *work)
{
	...
	unsigned char key_event_number_count = 0;
	...
	while(key_event_counter) {
		      ...
		      key_event_number_count = key_event & KEY_INDEX;
		      if( (0 <= key_event_number_count)  && (9 >= key_event_number_count))
		      {
				while(--key_event_counter)
				{
					key_event = i2c_smbus_read_byte_data(adp5587_data->client, KEY_EVENTA);
...
static int adp5587_input_device_init(struct i2c_client *client)
{
	...
	{
		client_data->keymap = keypad_keymap_m650;
		keynum = sizeof(keypad_keymap_m650)/sizeof(keypad_keymap_m650[0]);
