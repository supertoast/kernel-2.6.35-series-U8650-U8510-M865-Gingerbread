...
/*delete inputname for touch pane ID*/
		...
		if (ts->hasF11 && interrupt[ts->f11.interrupt_offset] & ts->f11.interrupt_mask) 
        {
            ...
            if(ts->is_support_multi_touch)
            {
                ...
                for (f = 0; f < ts->f11.points_supported; ++f) 
                {
                    ...
                    x = x * lcd_x / ts_x_max;
                    if(machine_is_msm7x27_u8650() || machine_is_msm7x27_c8650() || machine_is_msm7x27_m865() || machine_is_msm7x27_c8651())
                    {
...
/*delete inputname for touch pane ID, do not allocate*/
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* if the board id is m865 turn off the muti touch */
	/* the 8650 version G is the machine 8652 and turn off the muti touch */
    if(machine_is_msm7x27_m865() || (machine_is_msm7x27_u8650() && (get_hw_sub_board_id() == HW_VER_SUB_VG)))
    {
