...
static void gs_work_func(struct work_struct *work)
{
	...
	if(status & (1<<3))
	{
		u8 u8xl = reg_read(gs, GS_ST_REG_OUT_XL);
		u8 u8xh = reg_read(gs, GS_ST_REG_OUT_XH);
		u16x = (u8xh << 8) | u8xl;
		
		u8 u8yl = reg_read(gs, GS_ST_REG_OUT_YL);
		u8 u8yh = reg_read(gs, GS_ST_REG_OUT_YH);
		u16y = (u8yh << 8) | u8yl;
		
		u8 u8zl = reg_read(gs, GS_ST_REG_OUT_ZL);
		u8 u8zh = reg_read(gs, GS_ST_REG_OUT_ZH);
		u16z = (u8zh << 8) | u8zl;
