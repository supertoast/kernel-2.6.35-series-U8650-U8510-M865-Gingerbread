		...
		for (i = 0; i < diag_max_registration; i++) {
			if (driver->table[i].process_id != 0) {
		        ...
		        /* support automation diag command */
		          ...
		          && (driver->table[i].subsys_id == 0xF6) 
		          && (cmd_code == 0xF6)) 
				{
				  if (driver->table[i].cmd_code_lo <= subsys_id 
		           && driver->table[i].cmd_code_hi >= subsys_id)
		          {
					printk(KERN_INFO "automation: diag_process_apps_pkt cmd_code=%d, subsys_id=%d, lo=%d, hi=%d, i=%d\n", 
					driver->table[i].cmd_code, driver->table[i].subsys_id, driver->table[i].cmd_code_lo,
					driver->table[i].cmd_code_hi, i);
					driver->pkt_length = len;
					diag_update_pkt_buffer(buf);
					diag_update_sleeping_process(driver->table[i].process_id);
					return 0;
				  }
			    }/* end of else-if */
		        #endif
