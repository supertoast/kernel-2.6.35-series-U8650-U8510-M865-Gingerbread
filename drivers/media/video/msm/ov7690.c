...
static int ov7690_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	if (rc < 0)
	{	
		CDBG("ov7690_i2c_write 0x12 0x80 rc=%d", rc);
		goto init_probe_fail;
	}
    mdelay(5);
	...
	if (rc < 0)
	{
		CDBG("ov7690_i2c_read Model_ID failed!! rc=%d", rc);
		goto init_probe_fail;
	}
	...
	{
		CDBG("ov7690 Model_ID  error!!");
		rc = -ENODEV;
		goto init_probe_fail;
	}
    strncpy((char *)data->sensor_name, "23060036RF-OV-O", strlen("23060036RF-OV-O"));
