...
/*this is the all camera counts of the system!*/
...
#ifdef CONFIG_HUAWEI_CAMERA
/*send the camera counts of the system to user space*/
static int msm_get_sensor_counts(struct msm_sync *sync, void __user *arg)
{
	int rc = 0;
	unsigned int counts;

	if (copy_from_user(&counts,
			arg,
			sizeof(unsigned int)))
	{
		ERR_COPY_FROM_USER();
...
//		printk("sensor_mount_angle[i] -------->%d, i = %d", sensor_mount_angle[i], i);
		info.has_3d_support[i] = 0;
		info.is_internal_cam[i] = 0;
		info.s_mount_angle[i] = 180;//sensor_mount_angle[i];
		////////////ZHANGYU
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_SENSOR_INFO:
		...
		break;
    #ifdef CONFIG_HUAWEI_CAMERA
    case MSM_CAM_IOCTL_GET_SENSOR_COUNTS:
	   	rc = msm_get_sensor_counts(pmsm->sync, argp);
	   	break;
    #endif
			...
			struct msm_sensor_ctrl *))
{
	...
	/* If main(or slave) camera is probe, do not probe other camera */
