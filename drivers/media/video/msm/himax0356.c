...
static struct himax0356_i2c_reg_conf himax0356_init_reg_config[] =
{
    ...
    {0x19,0x81},
    {0x26,0x00},
    /*  Horizontal not mirror for the preview */ 
    {0x06,0x00},
    {0x22,0x07},
    {0x25,0x09},
    {0xAA,0x10},
...
static int himax0356_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	if (rc < 0)
	{
		CDBG("himax0356_i2c_write 0x21 0x00 rc=%d", rc);
		goto init_probe_fail;
	}
    mdelay(5);
	...
	if (rc < 0)
	{
		CDBG("himax0356_i2c_read Model_ID_H failed!! rc=%d", rc);
		goto init_probe_fail;   
	}
	...
	if (rc < 0)
	{
		CDBG("himax0356_i2c_read Model_ID_L failed!! rc=%d", rc);
		goto init_probe_fail;
	}
	...
	{
		CDBG("himax0356 Model_ID  error!!");
		rc = -ENODEV;
		goto init_probe_fail;
	}

    strncpy((char *)data->sensor_name, "23060041RF-Hm-F", strlen("23060041RF-Hm-F"));
