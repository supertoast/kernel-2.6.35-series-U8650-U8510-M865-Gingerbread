...
static camera_af_mode s5k4cdgx_af_mode = AF_MODE_AUTO;
/* Set flag that indicate if camera is initialization */
static uint8_t s5k4cdgx_init_flag = false;
...
int32_t s5k4cdgx_set_default_focus(uint8_t af_step)
{
    ...
    af_step, s5k4cdgx_af_mode);

    if(s5k4cdgx_af_mode == af_step)
    {
    ...
    {	
        ...
        if(AF_MODE_MACRO == s5k4cdgx_af_mode)
        {
            s5k4cdgx_i2c_write_w(0x002A , 0x02C4);  
            s5k4cdgx_i2c_write_w(0x0F12 , 0x0000);
            msleep(100);
        }
        s5k4cdgx_i2c_write_w(0x002A , 0x02C2);
        s5k4cdgx_i2c_write_w(0x0F12 , 0x0004);		
        ...
        s5k4cdgx_af_mode = AF_MODE_AUTO;
    }
    else if(AF_MODE_MACRO == af_step)
    {
        s5k4cdgx_i2c_write_w(0x0028 , 0x7000);                    
        s5k4cdgx_i2c_write_w(0x002A , 0x02C4);  
        s5k4cdgx_i2c_write_w(0x0F12 , 0x00B0);
        msleep(100);
        s5k4cdgx_i2c_write_w(0x002A , 0x02C2);
        s5k4cdgx_i2c_write_w(0x0F12 , 0x0004);		
		
        s5k4cdgx_af_mode = AF_MODE_MACRO;
    }
    else if(AF_MODE_NORMAL == af_step)    
    {
        ...
        s5k4cdgx_af_mode = AF_MODE_NORMAL;
    }
    else
    {
          CDBG("s5k4cdgx_set_default_focus:error af mode\n");
    }
...
/* Delete function s5k4cdgx_set_mirror_mode(void),it is excrescent */ 
    ...
    switch (rupdate)
    {
        case UPDATE_PERIODIC:
            ...
            {             
                ...
                //continue AF command 
                if(AF_MODE_AUTO == s5k4cdgx_af_mode || RES_CAPTURE  !=  last_rt)
                {	
                    s5k4cdgx_i2c_write_w(0x0028 , 0x7000);
            ...
            else
            {
                ...
                /* Delete some codes for take picture AF > */ 
                s5k4cdgx_i2c_write_w(0x0028 , 0x7000);
                s5k4cdgx_i2c_write_w(0x002A , 0x02C2);  //REG_TC_AF_AfCmd
        ...
        case REG_INIT:           
            ...
            {
                CDBG("%s: model is %d init sensor!", __FUNCTION__, s5k4cdgx_model_id);
...
static int s5k4cdgx_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
    ...
    strncpy((char *)data->sensor_name, "23060050SA-SAM-S", strlen("23060050SA-SAM-S"));
    ...
    struct msm_sensor_ctrl *s)
{
    ...
    * in the board file immediately. */
    ...
    if (rc < 0)
    {
        CDBG("camera sensor s5k4cdgx init  is failed!!!\n");
        goto probe_done;
    }
    else
    {
        s5k4cdgx_init_flag = true;
        CDBG("camera sensor s5k4cdgx init is succeed!!!\n");
    }
    #ifdef CONFIG_HUAWEI_HW_DEV_DCT
