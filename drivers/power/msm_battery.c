...
#define HUAWEI_PROCEDURE_CONSUEM_CURRENT_NTIFY 13
#endif
...
/* del the macro CONFIG_HAS_EARLYSUSPEND */
...
static void msm_batt_update_psy_status(void)
{
	...
	unnecessary_event_count = 0;
        ...
        #ifdef CONFIG_HUAWEI_KERNEL
        DBG_LIMIT("msm_batt_update_psy_status , begin, c_st, c_tp | b_st, b_lvl, b_vlt, b_tmp | batt_status :\nBATVAL, BEG:%d, %d | %d, %d, %d, %d |%d. \n",
		 charger_status, charger_type, battery_status, battery_level, 
		 battery_voltage, battery_temp, msm_batt_info.batt_status);
        #else
	...
	msm_batt_info.battery_temp 	= battery_temp;
        ...
        #ifdef CONFIG_HUAWEI_KERNEL
        DBG_LIMIT("msm_batt_update_psy_status , end: c_st, c_tp | b_st, b_lvl, b_vlt, b_tmp | batt_status :\nBATVAL, END: %d, %d | %d, %d, %d, %d |%d. \n",
		 msm_batt_info.charger_status, msm_batt_info.charger_type, msm_batt_info.battery_status, msm_batt_info.battery_level, 
		 msm_batt_info.battery_voltage, msm_batt_info.battery_temp, msm_batt_info.batt_status);
        #endif

	if (msm_batt_info.battery_voltage != battery_voltage) {
		msm_batt_info.battery_voltage  	= battery_voltage;
		msm_batt_info.batt_capacity =
        /* get the battery level from the modem side */
